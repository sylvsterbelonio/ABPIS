Public Class frmDistricts
    ...
    Private Sub reloadRecord()
        ...
        mydata = mySql.Query("SELECT tbl_group.GID,concat(surName,', ',firstname,' ' , middlename) as 'name' FROM  tbl_group  INNER JOIN tbl_districtname_pastoralleaders   ON (tbl_group.GID = tbl_districtname_pastoralleaders.GID)  INNER JOIN tblp_info    ON (tblp_info.IID = tbl_group.leader) where tbl_districtname_pastoralleaders.DID = " + DID)
                ...
                With frm
                    ...
                    If .returnValue <> "" Then
                        ...
                        If Not blnfound Then
                            Dim mydata As DataTable = mySql.Query("SELECT tbl_group.GID,concat(surName,', ',firstname,' ' , middlename) as 'name' FROM  tbl_group  INNER JOIN tbl_districtname_pastoralleaders   ON (tbl_group.GID = tbl_districtname_pastoralleaders.GID)  INNER JOIN tblp_info    ON (tblp_info.IID = tbl_group.leader) where tbl_districtname_pastoralleaders.GID = " + .returnValue)
                            dgvPastoralLeaders.Rows.Add()
                            dgvPastoralLeaders.Rows(dgvPastoralLeaders.RowCount - 2).Cells(2).Value = .returnValue
                            dgvPastoralLeaders.Rows(dgvPastoralLeaders.RowCount - 2).Cells(3).Value = mydata.Rows(0).Item(1)
                        Else
